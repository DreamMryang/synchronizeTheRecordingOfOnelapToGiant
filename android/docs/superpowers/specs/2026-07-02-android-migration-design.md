...
2. **设置**：顽鹿账号/密码、捷安特账号/密码（加密存储，修改后清对应 token 缓存）、同步天数（默认 30）、同步间隔（默认 6 小时，可选关闭以停用自动同步）、网络约束（默认任意网络）。账号密码在独立弹窗中编辑，确定即保存并触发调度重排。数据管理：清空同步历史（删除全部本地会话与记录，含本地 FIT 缓存；不影响捷安特服务端数据与去重）。
