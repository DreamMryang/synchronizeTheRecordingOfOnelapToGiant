...
- 用 SQLite 单表替代 txt，`fit_url` 唯一索引做去重，写入具备事务原子性。
...
| `fit_url` | TEXT NOT NULL UNIQUE | 顽鹿 fitUrl，去重键 |
...
去重规则：凡 `fit_url` 已存在**任意**记录（含 `*_FAILED`）的活动，下次任务一律跳过。
...
- `Set<String> findAllFitUrls()`：取所有已存在 fitUrl，供去重（替代读 txt）。
- `insertDownloaded(fitUrl, account, size)`
- `markDownloadFailed(fitUrl, account, err)`
- `markSynced(List<String> fitUrls)`
- `markUploadFailed(List<String> fitUrls, err)`
- 连接方式：SQLite 文件 + WAL，单连接常驻（Quartz `@DisallowConcurrentExecution` 已保证单写者），不引入连接池。

改造：

- `OnelapService.downloadTheOnelapFitFile()`：去重由 `dao.findAllFitUrls()` 提供；下载成功 `insertDownloaded`，下载失败 `markDownloadFailed` 后 `continue`。
